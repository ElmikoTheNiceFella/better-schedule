...
const Schedule = ({ scheduleData, isRamadan }) => {
                    ...
                    <div key={course.timing[0] + `${k} color`} style={{ border: "1px solid #343a40", top: (course.margin * 100) - minTimingMargin - 80, display: (j*13)+k == activeCourse ? "flex" : "none", flexWrap: "wrap" }} className={styles.colorChange}>
                      {colors.map((color, idx) => 
                        <div onClick={() => handleColorChange(course.name, course.type, idx)} 
                            key={color} 
                            style={{ backgroundColor: color, width: 38, height: 38, border: "1px solid #343a40" }}></div>)}
                        </div>
        ...
        <div className={styles.timesContainer}>
          {times.map((time, i) => 
            <div key={time[0]+`${i}`} 
                ...
                style={{ height: time[1], top: i * 100, borderBottom: times.length == 3 ? "1px solid #6c757d" : "none" }}>
                  {time[0]}
            </div>
          )}
        </div>
